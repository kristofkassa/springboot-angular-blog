...
import com.cj.blog.TechBlogApp;
import com.cj.blog.config.TestSecurityConfiguration;
import com.cj.blog.domain.Entry;
import com.cj.blog.repository.EntryRepository;
 ...
 */
@SpringBootTest(classes = { TechBlogApp.class, TestSecurityConfiguration.class })
...
public class EntryResourceIT {
    ...
    public void createEntry() throws Exception {
        ...
        restEntryMockMvc
            .perform(
                post("/api/entries").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(entry))
            )
    ...
    public void getAllEntries() throws Exception {
        ...
        restEntryMockMvc
            .perform(get("/api/entries?sort=id,desc"))
    ...
    public void getAllEntriesWithEagerRelationshipsIsEnabled() throws Exception {
        ...
        restEntryMockMvc.perform(get("/api/entries?eagerload=true")).andExpect(status().isOk());
    ...
    public void getEntry() throws Exception {
        ...
        // Get the entry
        restEntryMockMvc
            .perform(get("/api/entries/{id}", entry.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void updateNonExistingEntry() throws Exception {
        ...
        restEntryMockMvc
            .perform(
                put("/api/entries").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(entry))
            )
            .andExpect(status().isBadRequest());
    ...
    public void deleteEntry() throws Exception {
        ...
        // Delete the entry
        restEntryMockMvc
            .perform(delete("/api/entries/{id}", entry.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
